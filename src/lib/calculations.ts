...
export function computeMonthlyRevenue(
  ...
  for (let i = 0; i < 12; i++) {
    ...
    const classify = (e: PLSupplierInvoice) =>
      classifyByAccountCode(categoryMap.get(e.id), settings.cogsAccountPrefixes, settings.payrollAccountPrefixes)
    ...
    const classifyPrev = (e: PLSupplierInvoice) =>
      classifyByAccountCode(prevCategoryMap.get(e.id), settings.cogsAccountPrefixes, settings.payrollAccountPrefixes)
  ...
  let cumPayroll = 0, cumExt = 0, cumEbe = 0
  ...
  for (const m of months) {
    ...
    cumPayroll += m.payroll; cumExt += m.externalCosts; cumEbe += m.ebe
    ...
    m.cumPayroll = cumPayroll; m.cumExternalCosts = cumExt; m.cumEbe = cumEbe
  ...
  if (prevInvoices.length > 0) {
    ...
    if (settings) {
      prevFullDirectCosts = prevExpenses
        .filter((e) => classifyByAccountCode(prevCategoryMap.get(e.id), settings.cogsAccountPrefixes, settings.payrollAccountPrefixes) === 'cogs')
...
export function computeExpenseSummary(
  ...
  let totalPayrollFromInvoices = 0, totalDirectCosts = 0, totalExternalCosts = 0
